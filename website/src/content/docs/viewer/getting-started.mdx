...
PDF per locale, resolved through your bundler so it ships inside your build,
in the language negotiated from the viewer's locale and the browser's. The
...
A self-hosted URL never falls back to a CDN. Libraries a user creates —
stamps made from selected annotations, PDFs imported into the panel — are
kept in the browser across reloads.
