...
identifiers with translated labels. Its `urls` entry resolves the files
through your bundler, so they ship inside your build:

```ts
import { LOCALES, urls } from '@embedpdf/default-stamps/urls';
import { negotiateLocale } from '@embedpdf/react/i18n';

const locale = negotiateLocale(LOCALES, navigator.languages) ?? 'en';
const bytes = new Uint8Array(await (await fetch(urls[locale])).arrayBuffer());
await stamp.importLibraryPdf(bytes); // "Standaard stempels", id embedpdf-standard
```

Import two locales and you get two libraries with one identity: the
placed `/Name` is `Approved` either way, only the `/Subj` differs.
